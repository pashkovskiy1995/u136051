...
//Получение текущего месяца и года
    for ($i = 1; $i <= 12; $i++)
    {
        ...
        $arResult['PERIOD_ITEMS'][] = array('SELECTED' => ($_REQUEST['UF_PERIOD'] == '01.' . $i . '.' . date('Y')) ? 'selected' : '', 'VALUE' => '01.' . $i . '.' . date('Y'));
    }
###
###Получение списка показателей KPI для сотрудника###
$arResult['ITEMS'] = KPI\KPIManager:: GetKPIEmployee ($arParams [ "USER_ID" ]);
foreach ($arResult['ITEMS'] as $arItem) {
    $arResult['VALUES'][$arItem['ID']] = KPI\KPIManager::GetKPIEmployeeValue($arItem['ID'], $arParams ["USER_ID"], $_REQUEST['UF_PERIOD']);
}
